...
class Pager {
	...
	.Yin_pages {font:12px/1.6em Helvetica, Arial, sans-serif;overflow:hidden; text-align:center; font-family:Verdana;margin-bottom:5px;margin-top:20px;}
	.Yin_pages a, .pages{ margin:0 1px; padding:1px 6px; border:1px solid #E4E4E4; text-decoration:none!important; color:#666;}
	.Yin_pages a:hover { border-color:#369; }
	.Yin_pages strong { margin:0 1px; padding:2px 6px; border-color:#369; background:#369; color:#FFF; text-decoration:none!important; }
	.Yin_pages .back { padding:4px 6px 1px 20px!important; padding:4px 6px 2 20px;  font-family:simsun; }
	.Yin_pages .next { padding:4px 20px 1px 6px!important; padding:4px 20px 2 6px; font-family:simsun; }
	.Yin_pages .first { padding:4px 6px 1px 4px!important; padding:4px 6px 2 4px;  font-family:simsun; }
	.Yin_pages .last { padding:4px 4px 1px 6px!important; padding:4px 4px 2 6px; font-family:simsun; }
	</style>';
     ...
     * @param $size:每页显示多少条
     * @param $url
     * @param bool $default_style
     * @return string
     */
    public function pager($count, $size, $url, $default_style = false) {
        $count    = (int) $count;
        $size     = (int) $size;
        $page_num = ceil($count / $size); //总共多少页
        $page     = (int) $_GET['page'];
    ...
    private function pager_html($page_num, $url, $page, $default_style) {
        ...
        if ($default_style == true) {
            $html = $this->style_config . "<ul class='middle pagination'>";
        } else {
            $html = "<ul class='middle pagination'>";
        }
    ...
    private function get_pager_num_html($start, $end, $url, $page) {
        ...
        for ($i=$start; $i<=$end; $i++) {
            if ($i == $page) {
                $html .= "<li class='active'><a href='#' >{$i}</li></li>";
            } else {
    ...
    private function get_first_last_html($page_num, $url) {
        ...
        $first = "<li><a href='{$url}&page=1' class='first'>首页</a></li>";
        $last  = "<li><a href='{$url}&page={$page_num}' class='last'>尾页</a></li>";
    ...
    private function get_pager_next_back_html($url, $page, $page_num) {
        ...
        $next_page = $page + 1;
        $next = "<li><a href='{$url}&page={$next_page}' class='next'>下一条</a></li>";
        if ($page == $page_num) $next = '';
        $back_page = $page - 1;
        $back = "<li><a href='{$url}&page={$back_page}' class='back'>上一条</a></li>";
        if ($page == 1) $back = '';
